...
func AuthTwitchInit(g *gin.Context, in *routes.ThirdPartyAuthInit) error {
...
func AuthTwitchCallback(g *gin.Context) error {
	...
	scope, ok := utils.OAuthScopeStringFromToken(token)
	if !ok {
		g.AbortWithStatus(http.StatusInternalServerError)
		return nil
	}
...
func AuthTwitchCheck(g *gin.Context) (*routes.ThirdPartyAuthCheck, error) {
	maybeUser, ok := g.Get("user")
